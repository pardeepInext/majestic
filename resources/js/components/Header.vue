<template>
  <nav
    ...
    :class="[scrollMenu.status && 'scroll-nav']"
    id='main-menu'
  >
    <div class="container" ref="menu">
      <Link class="navbar-brand d-inline-flex" :href="'/'"
        ><img
          class="d-inline-block"
          :src="`${asset}img/gallery/logo.png`"
          alt="logo"
      ...
      >
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item px-2">
            <Link class="nav-link fw-medium active" href="/women">Women</Link
            >
          </li>
          <li class="nav-item px-2">
            <Link class="nav-link fw-medium" href="/men">Men</Link>
          </li>
          <li class="nav-item px-2">
...
export default {
  setup() {
    const {asset} = main();
    const menu = ref(null);
    const scrollMenu = reactive({ status:false})
    onMounted(()=>{
      window.addEventListener('scroll',e => {
        let scroll = window.scrollY;
        scrollMenu.status =  scroll > 50 ?  true: false; 
      });
    })
